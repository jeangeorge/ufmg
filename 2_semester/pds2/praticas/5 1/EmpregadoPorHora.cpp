...
EmpregadoPorHora::EmpregadoPorHora(const string &primeiro, const string &ultimo,
  const string &cpf, int mes, int dia, int ano, double taxaHora, 
  double horasTrabalhadas) : Empregado(primeiro, ultimo, cpf, mes, dia, ano)
{
  setTaxa(taxaHora);
...
double EmpregadoPorHora::getTaxa() const
{
  return taxa;
}
...
double EmpregadoPorHora::getHoras() const
{
  return horas;
}
...
void EmpregadoPorHora::imprime() const
{
  ...
  Empregado::imprime();
  cout << "\ntaxa por hora: " << getTaxa() <<"; horas trabalhadas: " << getHoras();
}
